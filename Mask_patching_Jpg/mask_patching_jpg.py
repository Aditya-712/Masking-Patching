"""
This module handles the processing of binary mask images in JPG/PNG format.
...
- Support both JPG and PNG input formats
- Convert masks to binary format (0 and 255)
"""
...
def pad_mask(image_array: np.ndarray, patch_size: int) -> tuple:
    ...
    Args:
        image_array (np.ndarray): Input binary mask array
...
def create_mask_patches(input_image_path: str, patch_size: int, overlap_percent: float) -> tuple:
    """
    Creates overlapping patches from a binary mask image.
    
    Args:
        input_image_path (str): Path to the input mask image (JPG or PNG)
    ...
    # Convert to numpy array and ensure binary values (0 and 255)
    image_array = np.array(image)
    # Invert the mask values to get correct black/white mapping
    image_array = np.where(image_array > 127, 0, 255).astype(np.uint8)
...
def save_mask_patches(patches, output_folder, image_name):
    """
    Saves the mask patches as JPG files.
...
def reconstruct_mask(patches, image_size, patch_size, overlap_percent, pad_y=0, pad_x=0):
    """
    Reconstructs the original mask from patches.
    ...
    Returns:
        np.ndarray: Reconstructed binary mask array
    """
...
def save_reconstructed_mask(reconstructed_image, output_image_path):
    """
    Saves the reconstructed mask as a JPG file.
    
    Args:
        reconstructed_image (np.ndarray): The reconstructed mask array
        output_image_path (str): Path where the reconstructed mask will be saved
    """
...
def mask_patching(input_image_path, patch_size, output_folder, overlap_percent):
    """
    Main function to handle the mask patching process.
    
    Args:
        input_image_path (str): Path to input mask image (JPG or PNG)
        patch_size (int): Size of patches
        output_folder (str): Output directory for patches
        overlap_percent (float): Overlap percentage between patches
    """
    patches, img_size, pad_y, pad_x = create_mask_patches(input_image_path, patch_size, overlap_percent)
    os.makedirs(output_folder, exist_ok=True)
    
    # Get base name without extension and convert to jpg if it's png
    base_name = os.path.splitext(os.path.basename(input_image_path))[0]
    save_mask_patches(patches, output_folder, base_name)

    # Optional reconstruction (commented out by default)
    # reconstructed_image = reconstruct_mask(patches, img_size, patch_size, overlap_percent, pad_y, pad_x)
    # reconstructed_image_path = os.path.join(output_folder, f"reconstructed_{base_name}.jpg")
    # save_reconstructed_mask(reconstructed_image, reconstructed_image_path)
    # print(f"Reconstructed mask saved as '{reconstructed_image_path}'.")

if __name__ == "__main__":
    patch_size = 640
    
    src = '/home/computervision/Documents/gis/building_detection/unet/torch/aditya_test/Input_samples/jpg/palmtrees/image'
    dest = '/home/computervision/Documents/gis/building_detection/unet/torch/aditya_test/Mask_patching_Jpg/mask_patching_output'
